<template>
    <div class="layout-padding">
        ...
        <kToolbar toolbarlabel="COTIZACIÓN No: " :documentId="public_id" :redirectTo="`/${model}`" @click="$router.push(`/${model}/create`)" label2="NUEVA COTIZACIÓN" icon2="add_circle_outline"></kToolbar>

        <div class="row justify-center mobile-only">
            <div style="width: 500px; max-width: 90vw;">
                <q-fixed-position corner="top-right" :offset="[16, 16]">
                    <q-btn round color="secondary" @click="mobileMenu($refs)" v-ripple.mat icon="add" class="animate-pop" />
                </q-fixed-position>
            </div>
        </div>
...
export default {
    ...
    components: {
        datatableShow, QInput, QSlider,
        QBtn, QIcon, QTooltip, QItemSeparator,
        QFixedPosition, QFab, sendemail, kToolbar,
        QFabAction, QList, QModal, attachfiles, totals
    },
    created() {
        this.fetchData();
    },
    data() {
        return {
            path: `estimate/${this.$route.params.id}`,
            data: {},
            listprice: '',
            contactname: '',
            seller: '',
    ...
    methods: {
        ...
        email(refs) {
            var pathEmailData = 'estimate_EmailToCustomer';
            refs._sendEmail.open(this.$route.params.id, pathEmailData, this.model);
        },
        edit() {
            this.$router.push(`/${this.model}/${this.$route.params.id}/edit`)
        },
        clone() {
            this.$router.push(`/${this.model}/${this.$route.params.id}/clone`)
        },
        pdf() {
        ...
        fetchData() {
            ...
            axios.get(`/api/${this.path}`)
                .then(function(response) {
                    vm.$set(vm, 'data', response.data.estimate)
