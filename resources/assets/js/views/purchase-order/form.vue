<template>
    <div class="layout-padding">
        ...
        <q-toolbar class="mobile-only" inverted color="secondary">
            ...
            <div class="row ">
                ...
                <div class="col-sm-5">
                    <q-list style="border: 0;padding: 0">
                        ...
                        <q-input color="secondary" type="textarea" v-model="form.observations" stack-label="Observaciones" placeholder="No visible en el documento impreso" />

                        <q-field :error="checkIfFieldHasError(errors,'notes')" error-label="Este campo es obligatorio">
                            <q-input color="secondary" type="textarea" v-model="form.notes" stack-label="*Notas" placeholder="Visibles en el documento impreso" />
                        </q-field>

                        <q-input color="secondary" type="textarea" v-model="form.terms" stack-label="Términos y condiciones" placeholder="Visibles en la orden de compra" />
            ...
            <tbody>
                <tr v-for="(_detail, index) in form.detail">
                    ...
                    <td data-th="PRODUCTO" style="width: 12em">
                        <q-field :error="checkIfFieldHasError(errors,['detail.' + index + '.category_id'])" error-label="Seleccione un producto">
                            <q-select class="no-margin no-padding" filter filter-placeholder="Buscar producto" v-model="_detail.category_id" :options="base.products" />
                        </q-field>
                    </td>
...
export default {
    ...
    data() {
        return {
            ...
            toolbarlabel: 'NUEVA ORDEN DE COMPRA No: ',
    ...
    created() {
        if (this.$route.meta.mode === 'edit') {
            this.path = `purchase-order/${this.$route.params.id}/edit`
            this.store = `purchase-order/${this.$route.params.id}`
            this.method = 'put'
            this.button = 'Guardar'
            this.toolbarlabel = 'EDITAR FACTURA DE VENTA No: '
            this.processType = "edit"
        } else if (this.$route.meta.mode === 'clone') {
            this.path = `purchase-order/${this.$route.params.id}/edit?convert=clone`
            this.store = `purchase-order`
            this.method = 'post'
    ...
    methods: {
        loadFiles(refs) {
            refs._attachfile.open(this.$route.params.id, 'purchase-order');
        },
