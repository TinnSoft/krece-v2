<template>
    <div class="layout-padding">
        ...
        <q-toolbar inverted color="secondary">
            <q-toolbar-title>
                <h5>
                    <b>{{toolbarlabel}} {{form.public_id}}</b>
                </h5>
            </q-toolbar-title>
                ...
                <div class="col-sm-5">
                    <q-list style="border: 0;padding: 0">
                        <q-field :error="checkIfFieldHasError(errors,'customer_id')" error-label="Este campo es obligatorio">
                            <q-select filter filter-placeholder="Buscar" stack-label="*Proveedor" v-model="form.customer_id" :options="base.contacts" />
                        </q-field>
...
export default {
    ...
    data() {
        return {
            ...
            options: [],
            redirect: '/debit-note',
            toolbarlabel: 'NUEVA NOTA DEBITO No: ',
    ...
    created() {
        if (this.$route.meta.mode === 'edit') {
            this.path = `debit-note/${this.$route.params.id}/edit`
            this.store = `debit-note/${this.$route.params.id}`
            this.method = 'put'
            this.button = 'Guardar'
            this.toolbarlabel = 'EDITAR NOTA DEBITO No: '
            this.processType = "edit"
        } else if (this.$route.meta.mode === 'clone') {
            this.path = `debit-note/${this.$route.params.id}/edit?convert=clone`
            this.store = `debit-note`
            this.method = 'post'
            this.toolbarlabel = 'CLONAR NOTA DEBITO No: '
    ...
    methods: {
        LoadFiles(refs) {
            refs._attachfile.open(this.$route.params.id, 'debit-note');
        },
