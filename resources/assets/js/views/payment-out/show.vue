<template>
    <div class="layout-padding">
        ...
        <q-toolbar inverted color="secondary">
            <q-toolbar-title>
                <h5>
                    <b>COMPROBANTE DE PAGO No: {{document_id}}</b>
                </h5>
            ...
            <q-btn class="within-iframe-hide" flat @click="$router.push(`/${model}/create`)">
                <q-icon name="add_circle_outline" />NUEVO GASTO
            </q-btn>
            ...
            <div class="row ">
                <div class="col-sm-5">
                    <q-list style="border: 0;padding: 0">
                        <q-input v-model="customerName" disable stack-label="Beneficiario" />
...
export default {
    ...
    data() {
        return {
            state: 'ABIERTA',
            model: 'payment-out',
            path: `payment-out/${this.$route.params.id}`,
            data: {},
            detail: [],
            total: 0,
            payment: '',
            isCategory: false,
            table: [],
            pathEmailData: 'getTemplateEmailToCustomerPaymentOut',
