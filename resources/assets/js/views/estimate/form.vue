...
import {
    ...
    QIcon, QTooltip, clone,
    QItemSeparator, QFixedPosition, QFab, QFabAction,
    QList, date, QSelect, QItemSide, QSpinner,
...
export default {
    ...
    data() {
        return {
            ...
            redirect: '/estimate',
            toolbarlabel: 'NUEVA COTIZACIÓN No: ',
    ...
    components: {
        ...
        Ripple, QSpinnerGears, attachfiles, totals, kToolbar
    },
    created() {
        if (this.$route.meta.mode === 'edit') {
            this.path = `estimate/${this.$route.params.id}/edit`
            this.store = `estimate/${this.$route.params.id}`
            this.method = 'put'
            this.button = 'Guardar'
            this.toolbarlabel = 'EDITAR COTIZACIÓN No: '
            this.processType = "edit"
        } else if (this.$route.meta.mode === 'clone') {
            this.path = `estimate/${this.$route.params.id}/edit?convert=clone`
            this.store = `estimate`
            this.method = 'post'
            this.toolbarlabel = 'CLONAR COTIZACIÓN No: '
    ...
    methods: {
        LoadFiles(refs) {
            refs._attachfile.open(this.$route.params.id, 'estimate');
        },
